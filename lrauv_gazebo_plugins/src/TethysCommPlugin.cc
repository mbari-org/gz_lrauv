  ...
  const gz::sim::EntityComponentManager &_ecm)
{
  ...
  //   * The MODEL frame is the vehicle nav frame -- its origin is the CTD inlet
  //     port, which is the point the LRAUV Main Vehicle Application means by
  //     "vehicle position".  Its axes are ENU-aligned (see the nav_to_body
  ...
  // The nav reference point is the CTD inlet, which is the model frame origin
  ...
  // referenced there.  The two points are ~0.54 m apart, so their velocities
  // differ by omega x r and the CoM velocity must be corrected to the nav point
